package org.modelio.module.attacktreedesigner.utils;

import com.modeliosoft.modelio.javadesigner.annotations.objid;

/**
 * @author ebrosse
 */
@objid ("63833be8-7915-45a2-9f83-cab9d9ca6238")
public interface IAttackTreeCustomizerPredefinedField {
    @objid ("b90035ec-0e81-40b1-af30-9a85bb435950")
    public static final String AND = "AND";

    @objid ("55583565-8387-4574-ac38-42c34254f985")
    public static final String OR = "OR";

    @objid ("156861c4-cfb1-474d-be87-c57610105de8")
    public static final String ATTACK = "Attack";

    @objid ("f60d625c-5161-4bb4-832d-c9cf24b9ade8")
    public static final String CONNECTION = "Connection";

    @objid ("b8c07ebc-0c7d-4970-afda-f6bae3e8460e")
    public static final String COUNTER_MEASURE = "CounterMeasure";

    @objid ("fbd9cab9-1a0a-497e-bfa7-ada6d197d165")
    public static final String TREE_REFERENCE = "Reference_Attack_Tree";

    @objid ("4db85201-28b6-4bc0-ab90-90c4260685a5")
    public static final String ANALYSE_THREAT = "Analyse_Threat";

    @objid ("e8524cca-0172-499a-9d52-e91cdd07fd64")
    public static final Object ATTACKTREE_STYLE_NAME = "attacktree";

}
